...
- `filter({ has/hasNot })` now accepts only Playwright `Locator` instances or registry path strings; inline locator strategy definitions and `{ locatorPath: ... }` wrappers are removed. Migration: replace inline definitions with registered paths or pass locators from `getLocator`/`getNestedLocator`.
