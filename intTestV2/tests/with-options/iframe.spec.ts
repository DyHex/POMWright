...
test.describe("iframe handling", () => {
	...
	test("getLocator finds iframeA and iframeB", async ({ iframePage }) => {
		const iframeA = await iframePage.getLocator("sectionA.frame");
		const iframeB = await iframePage.getLocator("sectionB.frame");
	...
	test("getLocator does not finds iframeC as it is inside iframeB", async ({ iframePage }) => {
		const iframeC = await iframePage.getLocator("sectionB.frame.innerFrame");

		await expect(iframeC).not.toBeVisible();
	});

	test("getNestedLocator finds iframeA and iframeB", async ({ iframePage }) => {
		const iframeA = await iframePage.getNestedLocator("sectionA.frame");
		const iframeB = await iframePage.getNestedLocator("sectionB.frame");
	...
	test("getNestedLocator finds nested iframeC", async ({ iframePage }) => {
		const iframeC = await iframePage.getNestedLocator("sectionB.frame.innerFrame");
		await expect(iframeC).toBeVisible();
	});

	test("toggle elements inside each iframe", async ({ iframePage }) => {
		const toggleA = await iframePage.getNestedLocator("sectionA.frame.toggle");
		const toggleB = await iframePage.getNestedLocator("sectionB.frame.toggle");
		const toggleC = await iframePage.getNestedLocator("sectionB.frame.innerFrame.toggle");
