...
test("locator definitions with options should resolve playground interactions", async ({ testFilters }) => {
	...
	const playgroundSection = await testFilters.nestedLocator("body.section@playground");

	const playgroundBtnRed = await testFilters.nestedLocator("body.section@playground.button@red");
